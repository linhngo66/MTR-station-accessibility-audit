...
def plot_facility_median(df, title="Median Availability Score by Facility"):
    # melt to long
    long = (
    ...
    med = med.sort_values("median_score")
...
plt.savefig('images/facility_score_median.png', dpi=300)
```
...
plot_facility_median(high, title="Median Availability Score by Facility — High Priority Stations")
plt.savefig('images/high_priority_facility_score.png', dpi=300)
```

```{python}
low = station_score[station_score["priority_bucket"].astype(str).str.lower().eq("low")]
plot_facility_median(low, title="Median Availability Score by Facility — Low Priority Stations")
plt.savefig('images/low_priority_facility_score.png', dpi=300)
```