...
fig.suptitle(
    "MTR stations by priority level, broken down by district",
    fontsize=10, fontweight="bold",
    x=0.06, y = 0.95, ha="left"
)
ax.set_title(
    "Bar width is proportional to the number of stations in the district",
    fontsize=8, color="dimgray",
    loc="left",                 # <-- left align
    pad=6,                      # <-- vertical gap from plot
)
ax.set_xlabel(None)
ax.set_ylabel("Percentage of stations in district", size = 8)
ax.set_yticks([])
...
plt.tight_layout()
plt.show()
plt.savefig('images/mosaic_district_priority.png', dpi=300)
```

## 3.2. Which facilities needs to be prioritized across all priority group?
...
# 2) Long format, 1 row per station x facility
long_tbl = (
    station_score[["station_name_en", "area", "priority_bucket"] + fac_cols]
      .melt(id_vars=["station_name_en", "area", "priority_bucket"],
            value_vars=fac_cols,
            var_name="facility_type", value_name="score_pct")
      .assign(
          facility_type=lambda d: d["facility_type"].map(label_map),
          priority_group=lambda d: d["priority_bucket"].astype(str).str.title()
      )[["station_name_en", "area", "priority_group", "facility_type", "score_pct"]]
)

# Save the file
long_tbl.to_csv('data/processed/facility_score.csv', index=False)
```